...
\mathcal{A}_{AB}\left[\phi_A\phi_B\psi(\ve{R})\right]\right\rbrace d\ve{r}^\text{\tiny internal}_{A,B}=0
\ee
...
\end{array}\;\;.
\ee
The parameter representation $\psi(\ve{R})=\int d\ve{R}\mprime\delta^{(3)}(\ve{R}-\ve{R}\mprime)\psi(\ve{R}\mprime)$
...
zero-range fermion-fermion -- \eg, \eftnopi~LO -- interaction are given for a two- and three-species system -- \eg, four neutrons assuming a
bound di-neutron, and 4-hydrogen, respectively. We denote the former
