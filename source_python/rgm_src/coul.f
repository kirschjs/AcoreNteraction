...
C     Intrinsic functions :   MIN, MAX, SQRT, DBLE, IMAG, ABS, LOG, EXP,
...
C
   20 IF(AXIAL) THEN
C                                                        REAL VERSION
      F = CF1R(X,ETA,ZLL,ACC8,SF ,RK, ETANE0,
     X         LIMIT,ERR,NFP,ACCH,FPMIN,FPMAX,PR,'COULCC')
          FCL = SF
          TPK1= RK
         ELSE
C                                                        COMPLEX VERSION
...
C  and
         Z  = TWO*P*X
C
         ZLLIN = DBLE(BB).LE.ZERO .AND. ABS(BB-NINTC(BB)).LT.ACC8**0.75
...
C
     ...
     X    .OR. ABS(ABS(DBLE(BB))-MB).LT.EP .AND. ABS(IMAG(BB)).LT.EP
      ...
      IF(J.GT.0 .OR. AT.GT.ATL .OR. I.GE.JMAX-2) J = J + 1
         IF(J.EQ.0) GO TO 10
         iish=i
         CALL RCF(X(1,1),X(1,2),J,iish,X(1,3),EPS)
              IF(iish.LT.0) GO TO 40
            DO 50 K=MAX(J,2),I
            D = ONE/(D*X(K,2) + ONE)
            DF = DF*(D - ONE)
            F = F + DF
...
C             GIVE COS(PSI)/COSH(IM(PSI)), WHICH ALWAYS HAS CORRECT SIGN
      ...
      IF(J.GT.0 .OR. AT.GT.ATL .OR. N.GE.NMAX-2) J = J + 1
         IF(J.EQ.0) GO TO 10
            nish=n
            CALL RCF(G,C,J,nish,XX,EPS)
              IF(nish.LT.0) GO TO 40
            DO 60 K=MAX(J,2),N
               D = ONE/(D*C(K) + ONE)
               DF = DF*(D - ONE)
               F = F + DF
...
C
      ...
      LOGICAL EVEN
      REAL*8 EPS
      COMMON /RCFCM2/ X1,x0,M2M1,MP12,EVEN,M
C     ibn = ibeg + inum - 1
      IBN = INUM
